...
public:
  ...
  void sweepFrequency(ChannelNum chan, uint32_t freq, bool follow = false)       // Target frequency
  {
    sweepDelta(chan, frequencyDelta(freq), follow);
  }

  void sweepDelta(ChannelNum chan, uint32_t delta, bool follow = false)
  {
    setChannels(chan);
  ...
  void sweepAmplitude(ChannelNum chan, uint16_t amplitude, bool follow = false)  // Target amplitude (half)
  {
    setChannels(chan);
  ...
  void sweepPhase(ChannelNum chan, uint16_t phase, bool follow = false)          // Target phase (180 degrees)
  {
    setChannels(chan);
  ...
  void Chirp(ChannelNum chan, uint32_t StartSweepFreq, uint32_t StopSweepFreq, uint16_t Increment=300, uint8_t Rate=250){
    //set starting frequency
    setFrequency(chan, StartSweepFreq);
    sweepRates(chan,Increment, Rate);
    //sweepDelta(chan,frequencyDelta(StartSweepFreq));
    update();
    sweepFrequency(chan,StopSweepFreq);
    
    //Sweep to stop frequency in no dwell and use SweepTime/getdelta? for values
    //sweepFrequency(chan,StopSweepFreq);
    //sweepRates(chan,Increment, Rate);
    //update();
  } //////////////////////////////////////// old function by Fermin. Very broken ////////////////////////////////
