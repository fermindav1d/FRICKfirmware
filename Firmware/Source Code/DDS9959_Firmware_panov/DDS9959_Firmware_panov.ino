...
// code to allow sweep functionality by Jacob Panov 3/20/23
//1.2 07/15/2021 fixed bug with phase drift during frequency tuning
//1.1 11/06/2020 fixed phase on outputs F2 and F3
//0.17 Monitoring frequency limits and displaying error messages on the screen
//0.16 changing the settings in the clock menu will be applied only after a forced save
// AD9959 library changed so that you can change the frequency through the SetClock reference_freq function
//0.15 the back button exits the clock settings menu
//0.14 10/19/2020 error correction
//0.13 Added saving clock settings in EEPROM
//0.12 Add saving the main settings to EEPROM
//0.11 we turn on the display of real values ​​​​in the DisplayMenu, we fix the error when the back button does not turn off the edit mode

#include  "AD9959_panov.h"
...
#define P0_PIN 16 // sweep pin specification 
#define P1_PIN 15
#define P2_PIN 14
#define P3_PIN 4
...
{ 
  ...
  uint32_t StartSweepFreq0 = 5135000UL; // initialize start frequency
  uint32_t StopSweepFreq0 = 5165000UL; // initalize stop frequency
  //dds.Chirp(MyAD9959::Channel0,StartSweepFreq0, StopSweepFreq0);
  
  // channel 0
  dds.setFrequency(MyAD9959::Channel0, StartSweepFreq0);
  dds.sweepRates(MyAD9959::Channel0,10,125,100,125); // rate is at 10 Hz per 1 us. 1 us is specified by the number 125

  //channel 1
  dds.setFrequency(MyAD9959::Channel1, StartSweepFreq0);
  dds.sweepRates(MyAD9959::Channel1,2,125,100,125); // changed 125 -> 10

  // channel 2
  dds.setFrequency(MyAD9959::Channel2, StartSweepFreq0);
  dds.sweepRates(MyAD9959::Channel2,10,35,100,125); 
  
  // channel 3
  dds.setFrequency(MyAD9959::Channel3, StartSweepFreq0);
  dds.sweepRates(MyAD9959::Channel3,2,125,100,125);

  //sweepDelta(chan,frequencyDelta(StartSweepFreq));
 // sweeping across all 4 channels
  //dds.sweepFrequency(MyAD9959::Channel0,StopSweepFreq0);
  dds.sweepFrequency(MyAD9959::Channel1,StopSweepFreq0);
  //dds.sweepFrequency(MyAD9959::Channel2,StopSweepFreq0);
  dds.sweepFrequency(MyAD9959::Channel3,StopSweepFreq0);
     dds.update(); // saving changes
     
  //digitalWrite(P0_PIN, HIGH); // 9959 checks for falling edge to initiate sweep function
  digitalWrite(P1_PIN, HIGH);
  //digitalWrite(P2_PIN, HIGH);
  digitalWrite(P3_PIN, HIGH);
  //delay(1); // can put a delay between sweeps
  digitalWrite(P0_PIN, LOW);
  digitalWrite(P1_PIN, LOW);
  digitalWrite(P2_PIN, LOW);
  digitalWrite(P3_PIN, LOW);
/*
  uint32_t StartSweepFreq1 = 2500000UL; 
  uint32_t StopSweepFreq1 = 2600000UL;
  dds.Chirp(MyAD9959::Channel1,StartSweepFreq1, StopSweepFreq1);
  */
}
