...
public:
  ...
  {
    sweepDelta(chan, frequencyDelta(freq));
  }
  ...
  void Chirp(ChannelNum chan, uint32_t StartSweepFreq, uint32_t StopSweepFreq, uint16_t Increment=10, uint8_t Rate=125){
    //set starting frequency
    setFrequency(chan,StartSweepFreq);
    setDelta(chan,frequencyDelta(StartSweepFreq));
    update();
